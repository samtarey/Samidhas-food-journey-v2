...
while(iWhile <= maxCount) {
	colName = tempColName + colCount + " .heading";
	...
	colName = tempColName + colCount + " .image";
...
//NOT WORKING - clikcing on arrow image and then trying to fading out the div images
//following code WORKING - tries to access name of recipe and recipe image from row-1, row-2, row-3 .... etc. and then fadeout that particular row when clicked on HEADINGS->maincourse-veg, non-veg, snacks etc.
//as the clicking on rightArrowImage not working, removed the arrow image. The following code of fading out is also not needed for now
/*function arrowClick() {
	$("#row1 .recipe-name").fadeOut("slow");
    $("#row1 .recipe-image").fadeOut("slow");

    	$("#recipeCol1").fadeIn("slow", function() {colCount = 1;
    		//alert(vegCount);
    	colName = tempColName + colCount;
		var formattedRecipeName = recipeName.replace("%data%", recipes.mainCourse.vegetarian[vegCount].name);
		$(colName).append(formattedRecipeName);
		var formattedRecipeImageSrc = recipeImg.replace("%data%", recipes.mainCourse.vegetarian[vegCount].recipeImages[0]);
		var formattedRecipeImage = formattedRecipeImageSrc.replace("%altdata%", recipes.mainCourse.vegetarian[vegCount].name);
		$(colName).append(formattedRecipeImage);
		vegCount++;
    });

$("#recipeCol2").fadeIn("slow", function() {colCount = 2;
    		//alert(vegCount);
    	colName = tempColName + colCount;
		var formattedRecipeName = recipeName.replace("%data%", recipes.mainCourse.vegetarian[vegCount].name);
		$(colName).append(formattedRecipeName);
...
$("#recipeCol3").fadeIn("slow", function() {colCount = 3;
    		//alert(vegCount);
    	colName = tempColName + colCount;
		var formattedRecipeName = recipeName.replace("%data%", recipes.mainCourse.vegetarian[vegCount].name);
		$(colName).append(formattedRecipeName);
...
$("#recipeCol4").fadeIn("slow", function() {colCount = 4;
    		//alert(vegCount);
    	colName = tempColName + colCount;
		var formattedRecipeName = recipeName.replace("%data%", recipes.mainCourse.vegetarian[vegCount].name);
		$(colName).append(formattedRecipeName);
